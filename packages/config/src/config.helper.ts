...
import { SnakeNamingStrategy } from './database/snakeNaming.config';
import {
  IAwsS3,
  IConfigOptions,
  IDBConfigs,
...
export const ConfigLoaderHelper = (
  ...
  envarFilePath?: string,
  workingDirectory?: string,
): IConfigOptions => {
  const { parsed } = dotenv.config({
    path: envarFilePath || process.cwd() + '/.env',
  });
  ...
  if (workingDirectory) {
    entities.push(`${workingDirectory}/**/*.entity{.ts,.js}`);
    mongoEntities.push(`${workingDirectory}/**/*.entity.mongo{.ts,.js}`);
  }
